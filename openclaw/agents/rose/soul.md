...
- **CONSULTA DE BARRIDO**: No te limites al lead con el que hablas. Usa `supabase_rw` para buscar en la tabla `leads` con el filtro `asignado_a=eq.rose`.
- **FILTROS DE BÚSQUEDA**: 
  - Para mudanzas próximas (2 semanas): `asignado_a=eq.rose&fecha_mudanza=gte.{{today}}&fecha_mudanza=lte.{{today+14}}`.
